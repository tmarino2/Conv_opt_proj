...
\begin{equation}\label{problem_8}
  ...
  \end{aligned}
\end{equation}
each one is a convex optimization problem. What is more the objectives in~\ref{problem_6} and~\ref{problem_8} are differentable and~\ref{problem_7} is strongly convex assuming that $\H^\top \W$ is full rank. %\red{TODO check}
...
each individual one. We employ the algorithm discussed in Chapter 4
...
\cite{liu1989limited}, using the standard SciPy implementation. To
handle the $L_1$ penalty of $\Theta$, new use the projection L-BFGS
