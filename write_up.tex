...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
    ...
    \STATE $\mu_{ij} ^{k+1} - \left(\mu_{ij}^k - \rho_k W_{ij}\right)$, $\forall i, j$
    \STATE $\eta_{ij}^{k+1} - \left(\eta_{ij}^k - \rho_k H_{ij}\right)$, $\forall i, j$
    \STATE $\xi_{i}^{k+1} - \left(xi_{i}^k - \rho_k \Theta_{i}\right)$, $\forall i$
    \STATE {\em STEP 3:}
    \STATE
    \STATE {\em STEP 4:}
    \STATE
